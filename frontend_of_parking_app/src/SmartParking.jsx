...
import { getSpots, getStatus, runDetection, getSettings, getAnalytics, API_BASE } from "./api.js";
...
const STATUS_META = {
  available: { label: "Available", dot: "bg-green-500", bg: "bg-green-50 dark:bg-green-900/30", text: "text-green-700 dark:text-green-400", border: "border-green-200 dark:border-green-800", tile: "bg-green-500" },
  occupied: { label: "Occupied", dot: "bg-red-500", bg: "bg-red-50 dark:bg-red-900/30", text: "text-red-700 dark:text-red-400", border: "border-red-200 dark:border-red-800", tile: "bg-red-500" },
  reserved: { label: "Reserved", dot: "bg-blue-600", bg: "bg-blue-50 dark:bg-blue-900/30", text: "text-blue-700 dark:text-blue-400", border: "border-blue-200 dark:border-blue-800", tile: "bg-blue-600" },
  unavailable: { label: "Unavailable", dot: "bg-gray-500", bg: "bg-gray-100 dark:bg-gray-800", text: "text-gray-600 dark:text-gray-400", border: "border-gray-300 dark:border-gray-700", tile: "bg-gray-500" },
};
...
function Logo({ dark, size = "md" }) {
  return (
    <div className="flex items-center gap-2">
      <div className={cx("rounded-xl bg-blue-600 flex items-center justify-center text-white", size === "md" ? "w-9 h-9" : "w-7 h-7")}>
        <ParkingCircle className={size === "md" ? "w-5 h-5" : "w-4 h-4"} />
      </div>
      <span className={cx("font-semibold tracking-tight", dark ? "text-white" : "text-gray-900 dark:text-white", size === "md" ? "text-lg" : "text-base")}>
...
function StatCard({ label, value, icon: Icon, tone, trend }) {
  ...
  return (
    <div className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm hover:shadow-md transition-shadow duration-300">
      <div className="flex items-center justify-between mb-3">
        <div className={cx("w-10 h-10 rounded-xl flex items-center justify-center", tones[tone])}>
          <Icon className="w-5 h-5" />
        </div>
        {trend !== undefined && (
          <span className={cx("flex items-center gap-0.5 text-xs font-medium", trend >= 0 ? "text-green-600" : "text-red-600")}>
...
function Modal({ title, onClose, children, wide }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className={cx("bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-h-[85vh] overflow-y-auto", wide ? "max-w-2xl" : "max-w-md")}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h3 className="font-semibold text-gray-900 dark:text-white">{title}</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500">
            <X className="w-4 h-4" />
          </button>
        </div>
...
const inputCls = "w-full px-3.5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-600/40 focus:border-blue-600 transition-all duration-200";
    ...
    <div className="space-y-4">
      {ROWS.map((row) => (
        <div key={row} className="flex items-center gap-3">
          <div className="w-7 shrink-0 text-sm font-semibold text-gray-400 dark:text-gray-500">{row}</div>
              ...
              return (
                <button
                  ...
                  className={cx(
                    ...
                    selectable && "hover:scale-105 hover:shadow-sm cursor-pointer"
                  )}
...
function TopBar({ dark, setDark, onLogout, title, role }) {
  return (
    <div className="sticky top-0 z-30 flex items-center justify-between gap-4 px-6 py-4 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800">
      <div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative hidden md:block">
          ...
          <input placeholder="Search…" className="pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200 w-56 focus:outline-none focus:ring-2 focus:ring-blue-600/30" />
        </div>
        <IconBtn icon={dark ? Sun : Moon} onClick={() => setDark(!dark)} label="Toggle theme" />
...
function Sidebar({ items, active, setActive, collapsed, setCollapsed, dark }) {
  return (
    <div className={cx(
      "hidden lg:flex flex-col shrink-0 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 transition-all duration-300",
      collapsed ? "w-20" : "w-64"
    )}>
      ...
      <button
        ...
        className="m-3 h-10 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"
      >
...
function MobileBottomNav({ items, active, setActive }) {
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 flex items-center justify-around px-1 py-2">
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => goto("login", "user")} className="text-sm font-medium text-gray-700 dark:text-gray-200 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
              User login
            </button>
      ...
      <section className="max-w-7xl mx-auto px-6 pt-16 pb-12 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <button onClick={() => goto("register")} className="px-5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
              Register as a user
            </button>
          </div>
        ...
        <div className="relative">
          <div className="rounded-3xl bg-gradient-to-br from-blue-50 to-white dark:from-blue-900/20 dark:to-gray-900 border border-gray-100 dark:border-gray-800 p-8 shadow-sm">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((f) => (
            <div key={f.title} className="p-5 rounded-2xl border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow duration-300 bg-white dark:bg-gray-900">
      ...
      {showGrid && (
        <Modal title="Live parking — visitor view" onClose={() => setShowGrid(false)} wide>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {stats.map((s) => (
              <div key={s.label} className="p-3 rounded-xl bg-gray-50 dark:bg-gray-900 text-center">
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-white dark:bg-gray-950">
      ...
      <div className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-sm">
            ...
            {["user", "admin"].map((r) => (
              <button
                ...
                className={cx(
                  ...
                  role === r ? "bg-white dark:bg-gray-700 text-blue-600 shadow-sm" : "text-gray-500 dark:text-gray-400"
                )}
              >
                {r}
              </button>
          ...
          <div className="mt-6 p-3 rounded-xl bg-gray-50 dark:bg-gray-800 text-xs text-gray-500 dark:text-gray-400">
...
function RegisterPage({ goto }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm p-8">
        ...
        <form onSubmit={(e) => { e.preventDefault(); goto("login", "user"); }}>
          <div className="grid sm:grid-cols-2 gap-x-4">
            <Field label="Full name"><input required className={inputCls} placeholder="Aarav Shrestha" /></Field>
            <Field label="College ID"><input required className={inputCls} placeholder="BCT-078-201" /></Field>
          </div>
          <div className="grid sm:grid-cols-2 gap-x-4">
            <Field label="Phone number"><input required className={inputCls} placeholder="98XXXXXXXX" /></Field>
            <Field label="Vehicle number"><input required className={inputCls} placeholder="BA 2 PA 3312" /></Field>
          </div>
          <div className="grid sm:grid-cols-2 gap-x-4">
            ...
            <Field label="Vehicle model"><input className={inputCls} placeholder="e.g. Hyundai i20" /></Field>
          </div>
        ...
        <div className="p-6 max-w-6xl mx-auto">
          ...
          {section === "layout" && (
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4 shadow-sm">
                <div className="relative flex-1 min-w-[160px]">
                  <Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
                  <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search slot, e.g. A3" className="pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-600/30" />
                </div>
                <select value={floorFilter} onChange={(e) => setFloorFilter(e.target.value)} className="px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm">
                  <option value="all">All floors</option>
                  <option value="1">Floor 1</option>
                  <option value="2">Floor 2</option>
                </select>
                <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm">
                ...
                {notifications.map((n) => {
                  ...
                  return (
                    <div key={n.id} className={cx("flex items-start gap-3 p-5", !n.read && "bg-blue-50/50 dark:bg-blue-900/10")}>
                      <div className={cx("w-9 h-9 rounded-xl flex items-center justify-center shrink-0", STATUS_META[n.kind]?.bg || "bg-gray-100 dark:bg-gray-700", STATUS_META[n.kind]?.text || "text-gray-500")}>
          ...
          {section === "notices" && (
            <div className="space-y-4">
              {notices.map((n) => (
                <div key={n.id} className={cx("bg-white dark:bg-gray-800 rounded-2xl border p-5 shadow-sm", n.emergency ? "border-red-200 dark:border-red-800" : "border-gray-100 dark:border-gray-700")}>
      ...
      {selectedSlot && (
        <Modal title={`Slot ${selectedSlot.id}`} onClose={() => setSelectedSlot(null)}>
          <div className="space-y-3 text-sm">
            ...
            <p className="text-xs text-gray-400 pt-2 border-t border-gray-100 dark:border-gray-700">Slot booking isn't available to users — reserved and emergency holds are set by campus admins only.</p>
          </div>
        </Modal>
      )}
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  {["Pending", "In Review", "Resolved", "Closed"].map((s) => (
                    <button
                      ...
                      className={cx("text-xs px-2.5 py-1 rounded-lg border", f.status === s ? "border-blue-600 text-blue-600" : "border-gray-200 dark:border-gray-700 text-gray-500")}
                    >
                      {s}
                    </button>
                  ))}
...
function ProfileSection() {
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <div className="md:col-span-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm text-center">
      ...
      <div className="md:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm space-y-4">
...
function SettingsSection() {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm max-w-lg space-y-1">
      ...
      {["Email notifications", "Push notifications", "Two-factor authentication"].map((s) => (
        <label key={s} className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700 last:border-0 text-sm text-gray-600 dark:text-gray-400">
          {s}
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200"
                >
                  ...
                  <div className="mt-6 space-y-3">
                    {[{ l: "Morning (7–10a)", v: 62 }, { l: "Midday (11a–2p)", v: 91 }, { l: "Evening (3–6p)", v: 70 }].map((x) => (
                      <div key={x.l}>
                        ...
                        <div className="h-2 rounded-full bg-gray-100 dark:bg-gray-700"><div className="h-2 rounded-full bg-blue-600" style={{ width: `${x.v}%` }} /></div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <Modal title={`Manage slot ${editSlot.id}`} onClose={() => setEditSlot(null)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-900"><p className="text-gray-400 text-xs">Floor</p><p className="text-gray-800 dark:text-gray-200 font-medium">{editSlot.floor}</p></div>
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-900"><p className="text-gray-400 text-xs">Type</p><p className="text-gray-800 dark:text-gray-200 font-medium">{editSlot.type}</p></div>
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="relative max-w-xs">
        <Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search users…" className="pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-600/30" />
      </div>
                ...
                <tr key={u.id}>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-end gap-2">
                      ...
                      <button className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400" title="Edit"><Edit2 className="w-4 h-4" /></button>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          <Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search vehicle or owner…" className="pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-600/30" />
        </div>
        <select value={filter} onChange={(e) => setFilter(e.target.value)} className="px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm">
          <option value="all">All statuses</option>
          <option value="Parked">Parked</option>
          <option value="Exited">Exited</option>
        </select>
        <button className="inline-flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
      ...
      <div className="flex justify-end gap-2 text-sm text-gray-500 dark:text-gray-400">
        <button className="w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center">1</button>
        <button className="w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center justify-center">2</button>
        <button className="w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center justify-center">3</button>
      </div>
    </div>
  );
}
...
function ReportsSection() {
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-5">
      {reports.map((r) => (
        <div key={r.title} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm">
          ...
          <div className="flex gap-2">
            {["CSV", "PDF", "Excel"].map((f) => (
              <button key={f} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-1">
    ...
    <div className="space-y-4">
      ...
      {notices.map((n) => (
        <div key={n.id} className={cx("bg-white dark:bg-gray-800 rounded-2xl border p-5 shadow-sm", n.emergency ? "border-red-200 dark:border-red-800" : "border-gray-100 dark:border-gray-700")}>
          ...
          <div className="flex gap-2">
            <button onClick={() => setNotices(notices.map((x) => x.id === n.id ? { ...x, pinned: !x.pinned } : x))} className="text-xs px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 flex items-center gap-1">
              <Pin className="w-3.5 h-3.5" /> {n.pinned ? "Unpin" : "Pin"}
            </button>
            <button className="text-xs px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 flex items-center gap-1">
